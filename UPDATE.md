# Apply the September 19 checkout update
...
3. In PowerShell, from your existing project folder, run:
...
**Stop if any command fails. Apply the database migration before deploying the new Worker.** The migration only adds notification/recovery fields and an index; it preserves existing orders. No new database is needed. If earlier paid website orders exist, Gwyn receives their first owner notification when maintenance next checks them.
...
- The checkout banner offers free shipping on the first 50 orders over $25. Merchandise must exceed $25 before tax. The least expensive available service is free; faster upgrades keep their full quoted shipping price. A $25.00 cart does not qualify; a $25.01 cart does.
- There is deliberately no order counter or automatic end date. To end the offer, change free_shipping.enabled to false in content/store.json, rebuild, and deploy. The banner and discount turn off together.
...
4. Compare a $25 cart with one over $25. Check the free service, any paid upgrade, one combined parcel, and 5.3% merchandise tax.

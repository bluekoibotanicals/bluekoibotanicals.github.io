...
export function squareOrder(env, order) {
  const {lines,address:a,totals:t,rate} = order;
  return {idempotency_key:order.id,order:{location_id:env.SQUARE_LOCATION_ID,reference_id:order.id,pricing_options:{auto_apply_discounts:false,auto_apply_taxes:false},line_items:lines.map(p=>({catalog_object_id:p.variation_id,quantity:String(p.quantity),base_price_money:{amount:p.price_cents,currency:'USD'}})),taxes:t.tax ? [{uid:'va-sales-tax',name:'Sales tax (5.3%)',percentage:'5.3',scope:'ORDER',type:'ADDITIVE'}] : [],service_charges:t.shipping ? [{name:`Shipping: ${rate.carrier} ${rate.service}`,amount_money:{amount:t.shipping,currency:'USD'},calculation_phase:'TOTAL_PHASE',taxable:false}] : [],fulfillments:[{type:'SHIPMENT',state:'PROPOSED',shipment_details:{recipient:{display_name:a.name,email_address:a.email,phone_number:a.phone || undefined,address:squareAddress(a)}}}]}};
}

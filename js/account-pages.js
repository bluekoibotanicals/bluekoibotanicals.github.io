...
if($('#order-details')){
  ...
  async function refresh(){
    try{const o=await api('api/order/'+encodeURIComponent(id || ''));
      ...
      $('#order-message').textContent=o.status==='paid'?'Gwyn will prepare your order in 1–3 days. Your order confirmation will be sent by email.':o.status==='refunded'?'Contact us if you have any questions about your refund.':o.status==='failed'?'You can return to your bag and get a fresh shipping quote.':'Please do not place another order while we confirm this payment.';
      ...
      if(o.status==='processing' && polls>=12)$('#order-message').textContent='Your payment still needs confirmation. Keep your order number and contact us before placing another order.';
...
if($('#admin-login')){
  ...
  const pastInvitations=await import('./past-reviews-admin.js');
  const invitationUI=pastInvitations.setupPastInvitations(headers);
  async function refresh(){
    ...
    try{
      ...
      $('#admin-reviews').innerHTML=reviews.reviews.length?reviews.reviews.map(r=>`<article class="admin-card"><p class="small muted">${esc(r.slug)} · ${esc(r.status)} · ${r.purchase_source==='website'?'Website purchase':'Past purchase: '+esc(r.purchase_source)}</p><h3>${esc(r.author)} · ${r.rating}/5</h3><p>${esc(r.body)}</p><label class="field">Moderation reason (required to reject)<input data-reason="${r.id}" maxlength="300" placeholder="For example: contains private contact details"></label><div class="actions"><button class="button" data-review="${r.id}" data-status="published">Publish</button><button class="button secondary" data-review="${r.id}" data-status="pending">Hold</button><button class="button secondary" data-review="${r.id}" data-status="rejected">Reject</button></div></article>`).join(''):'<p>No reviews submitted yet.</p>';

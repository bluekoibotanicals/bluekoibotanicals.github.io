...
After migration 0004, Admin → Invite past customers accepts a verified prior purchase, source/reference, date, customer details, and selected products. It sends a private invitation immediately without creating an order or changing stock. Links expire after 90 days; reviews require moderation. The invitation list allows copying and revoking a link. It shows the latest 100 invitations. Email retries use a frozen payload/key and stop after 23 hours; check Resend if automatic retries have stopped. See UPDATE.md for the complete workflow.

## Checkout promotion and market calendar
Migration 0004 creates FREESHIP25 if missing, without overriding an existing code. The checkout banner is visible only when that code is active, free-shipping, and has a $25.01 minimum. The customer must enter the code or click Use FREESHIP25 and then request shipping options. The first-50-customer limit remains manual; deactivate the code to end the promotion and hide its banner.

The 2026 market schedule lives in content/markets.json. The homepage calendar is generated by scripts/market_calendar.py and enhanced by js/site.js. Wednesdays end October 28; Saturdays end December 19 and skip November 7 and 14. All times are Eastern. Update the schedule and rebuild for future seasons.
